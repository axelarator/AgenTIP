...
def build_worklist(con: duckdb.DuckDBPyConnection,
    ...
    never = [r[0] for r in con.execute(
        """SELECT DISTINCT o.indicator_value FROM observations_wide o
           JOIN actors a ON a.actor_name = o.actor
           WHERE a.tracked AND o.indicator_type IN ('ipv4', 'ipv6')
             AND NOT EXISTS (
               SELECT 1 FROM observations_wide e
    ...
    stale = [r[0] for r in con.execute(
        """SELECT e.indicator_value FROM observations_wide e
           JOIN observations_wide o ON o.indicator_value = e.indicator_value
           JOIN actors a ON a.actor_name = o.actor
    ...
    rdap_fresh = {r[0] for r in con.execute(
        """SELECT indicator_value FROM observations_wide
